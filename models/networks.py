...
class ELM(nn.Module):
    def __init__(self, num_of_hidden, num_classes):
        super(ELM, self).__init__()
        self.num_of_hidden = num_of_hidden
        self.num_classes = num_classes

    def train(self, feature, label, C):
        dataset_size = feature.size()[0]
        input_nodes = feature.size()[1]
        #convert label to one_hot
        one_hot = np.zeros((dataset_size, self.num_classes))
        one_hot[np.arange(dataset_size), label.numpy()] = 1.0
        one_hot_label = torch.from_numpy(one_hot).to(torch.float32) #torch tensor

        #print('**Training ELM**')
        '''
        dim_max = max(input_nodes, self.num_of_hidden)
        input_weights_ = ortho_group.rvs(dim_max)
        if input_nodes >= self.num_of_hidden:
            self.input_weights = torch.from_numpy(input_weights_[:, 0:self.num_of_hidden]).to(torch.float32)
        if input_nodes < self.num_of_hidden:
            self.input_weights = torch.from_numpy(input_weights_[0:input_nodes, :]).to(torch.float32)
        '''
        self.input_weights = torch.from_numpy(np.random.randn(input_nodes, self.num_of_hidden)).to(torch.float32)
        #b = np.random.randn(self.num_of_hidden)
        #self.bias = torch.from_numpy(b / np.linalg.norm(b, ord=2)).to(torch.float32)
        self.bias = torch.from_numpy(np.random.randn(self.num_of_hidden)).to(torch.float32)
        tem_H = torch.matmul(feature, self.input_weights) + self.bias
        H = F.leaky_relu(tem_H)
        H_inverse = operations.compute_pinv(H, True, C)
        del H
        gc.collect()
        self.beta = torch.matmul(H_inverse, one_hot_label)
        del H_inverse
        gc.collect()

    '''
    def train_no_random_projection(self, H, label, C):
        dataset_size = H.size()[0]
        # convert label to one_hot
        one_hot = np.zeros((dataset_size, self.num_classes))
        one_hot[np.arange(dataset_size), label.numpy()] = 1.0
        one_hot_label = torch.from_numpy(one_hot).to(torch.float32)  # torch tensor
    ...
    def test(self, feature, label):
        dataset_size = feature.size()[0]
        cls_sum = np.zeros(self.num_classes)
        correct_sum = np.zeros(self.num_classes)
        #print('**Testing ELM**')
        H = F.leaky_relu(torch.matmul(feature, self.input_weights) + self.bias)
        pred = torch.matmul(H, self.beta).numpy()
        del H
        gc.collect()
        self.pred_label = np.argmax(pred, axis=1)
        # compute instance accuracy
        self.correct = np.sum(self.pred_label == label.numpy())
        self.accuracy = self.correct/float(dataset_size)*100
        #print('The ELM instance accuracy is: %.2f%%' % self.accuracy)
        for idx in range(dataset_size):
            cls_sum[label[idx].item()] += 1
            if self.pred_label[idx] == label[idx].item():
                correct_sum[label[idx].item()] += 1
        self.cls_accuracy = correct_sum.astype(np.float32) / cls_sum.astype(np.float32)
        self.mean_cls_accuracy = np.mean(self.cls_accuracy) * 100
    '''
    def test_no_random_projection(self, H, label):
        dataset_size = H.size()[0]
        cls_sum = np.zeros(self.num_classes)
        correct_sum = np.zeros(self.num_classes)
        # print('**Testing ELM**')
...
class ELM_AE(nn.Module):
    def __init__(self, num_hidden, feature, C):
        super(ELM_AE, self).__init__()
        self.num_hidden = num_hidden
        self.feature = feature
        self.input_dim = self.feature.size()[1]
        self.C = C

    def autoencoder(self):
        dim_max = max(self.input_dim, self.num_hidden)
        A_ = ortho_group.rvs(dim_max)
        if self.input_dim >= self.num_hidden:
            self.A = torch.from_numpy(A_[:, 0:self.num_hidden]).to(torch.float32)
        if self.input_dim < self.num_hidden:
            self.A = torch.from_numpy(A_[0:self.input_dim, :]).to(torch.float32)
        #np.random.seed(self.seed)
        #self.A = torch.from_numpy(np.random.rand(input_dim, self.num_hidden)).to(torch.float32)
        b = np.random.randn(self.num_hidden)
        self.b = torch.from_numpy(b/np.linalg.norm(b, ord=2)).to(torch.float32)
        #print('start multiply')
        t1 = time.time()
        H = F.leaky_relu(torch.matmul(self.feature, self.A) + self.b)
        t2 = time.time()
        #print(t2-t1)
        #print('start compute inverse')
        H_inverse = operations.compute_pinv(H, True, self.C)
        t3 = time.time()
        del H
        gc.collect()
        #print(t3-t2)
        self.beta = torch.matmul(H_inverse, self.feature)
        del H_inverse
        gc.collect()

    def feature_extractor(self, feature):
        self.new_feature = F.leaky_relu(torch.matmul(feature, self.beta.t()))
        return self.new_feature

class ELM_AE_3D(nn.Module):
    def __init__(self, num_hidden, feature, C):
        '''
        :param num_hidden:
        :param feature: The feature is of 3 dimension that has the size of [batch, N, d]
        :param C:
        :param random_seed:
        '''
        super(ELM_AE_3D, self).__init__()
        self.num_hidden = num_hidden
        self.feature = feature
        self.dataset_size, self.num_points, self.input_dim = self.feature.size()
        self.C = C
    ...
    def autoencoder(self):
        ...
        if self.input_dim < self.num_hidden:
            self.A = torch.from_numpy(A_[0:self.input_dim, :]).to(torch.float32)
        #np.random.seed(self.seed)
        #self.A = torch.from_numpy(np.random.rand(self.input_dim, self.num_hidden)).to(torch.float32)
        b = np.random.randn(self.num_hidden)
        self.b = torch.from_numpy(b/np.linalg.norm(b, ord=2)).to(torch.float32)
        #print('start multiply')
        #t1 = time.time()
        H = F.leaky_relu(torch.matmul(self.feature, self.A) + self.b)
        H.resize_(self.dataset_size*self.num_points, self.num_hidden)
        #t2 = time.time()
        #print(t2 - t1)
        #print('start compute inverse')
        H_inverse = operations.compute_pinv(H, True, self.C)
        ...
        gc.collect()
        #print(t3 - t2)
        self.beta = torch.matmul(H_inverse, torch.reshape(self.feature, (-1, self.input_dim)))
        del H_inverse
        gc.collect()

    def feature_extractor(self, feature):
        '''
        :param feature: 3-d tensor of shape (batch, N, d)
        :param mode: train or test
        :return: new feature of shape (batch, N, L)
        '''
        #if mode == 'train':
        self.new_feature = F.leaky_relu(torch.matmul(feature, torch.transpose(self.beta, 0, 1)))
        #if mode == 'test':
            #batch_size = self.feature.size()[0]
            #beta = torch.zeros(self.num_hidden, self.input_dim, dtype=torch.float32)
            #for i in range(batch_size):
                #beta += self.beta[i]/batch_size
            #self.new_feature = F.relu(torch.matmul(feature, beta.t()))

        return self.new_feature

class ELM_AE_L1(nn.Module):
    def __init__(self, num_hidden, feature, lmbda):
        super(ELM_AE_L1, self).__init__()
        self.num_hidden = num_hidden
        self.feature = feature
        self.lmbda = lmbda
    ...
    def autoencoder(self):
        ...
        self.A = torch.randn(input_dim, self.num_hidden, dtype=torch.float32)
        b = np.random.randn(self.num_hidden)
        self.b = torch.from_numpy(b/np.linalg.norm(b, ord=2)).to(torch.float32)
        H = F.relu(torch.matmul(self.feature, self.A) + self.b)
        self.beta = operations.ELM_L1_reg(H, self.feature, self.lmbda)

    def feature_extractor(self, feature):
        self.new_feature = F.relu(torch.matmul(feature, self.beta.t()))
        return self.new_feature
